...
stats <- read.csv("gaez/data/gaez_stats_high.csv")
...
write.csv(met1, "gaez/data/cats_high.csv")
...
stats <- stats[-c(2,12)]
names(stats)[10] <- "full_name"

# Match with mapspam name
m <- sort(unique(mapspam$fullname))
g <- sort(unique(stats$Crop.name))
...
# wide format
high <- read.csv("gaez/gaez_high.csv")
...
write.csv(ff, "gaez/output/gaez_high.csv")
